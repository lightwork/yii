...
class Image {
	...
	// Borrowed from 
	// http://nerdblog.pl/2009/11/11/kohana-php-adding-watermark-with-png-24-support/
	// and
	// http://kohanaframework.org/3.0/guide/api/Kohana_Image_GD#watermark
	public function watermark(Image $watermark, $offset_x = NULL, $offset_y = NULL, $transparency = 100) 
    {
	    if ($offset_x === NULL)
	    {
	        // Center the X offset
	    ...
	    elseif ($offset_x === self::OFFSET_LEFT)
	    {
	        $offset_x = 0;
	    }
	    elseif ($offset_x === self::OFFSET_RIGHT)
	    {
	        // Set the X offset from the right
	        $offset_x = $this->width - $watermark->width;
	    }
	    ...
	    elseif($offset_y === self::OFFSET_BOTTOM)
	    {
	        // Set the Y offset from the bottom
	        $offset_y = $this->height - $watermark->height;
	    }
	    elseif($offset_y === self::OFFSET_TOP)
	    {
	        // Set the Y offset from the bottom
	        $offset_y = 0;
	    }
	    ...
	    $transparency = min(max($transparency, 1), 100);
